    ...
    return (
        <div  className=' h-[568px] w-full relative'>
            ...
            <motion.div animate={{ opacity: [0, 100] }}
                    transition={{ duration: 2, ease: "linear" }}  className={`absolute w-[80%] -bottom-10   left-1/2 gap-3 -translate-x-1/2 rounded-md flex flex-col  ${lang === "ar" ? "md:flex-row-reverse" : "md:flex-row"}  items-center bg-[#EEEEEE] p-3`}>
                <div  className={`w-full md:basis-1/4 ${lang === "ar" ? "text-right border-l-2" : "text-left border-r-2"} border-neutral-400`}>
                ...
                <div  className={`w-full md:basis-1/4 ${lang === "ar" ? "text-right" : "text-left"}`}>
                    <p className='text-[16px] text-neutral-400'>{t('Choose')}</p>
                ...
                <motion.svg  animate={{ x: [0, 10, 0] }}
                    ...
                    <Link className='w-24 flex justify-center items-center' to="/mytrips"><div className='p-1 flex items-center h-full  rounded-md text-[14px] bg-slate-700 text-white font-bold'>{t('View Flights')}</div></Link>
                    </div>
            </motion.div>
        </div>
    );
